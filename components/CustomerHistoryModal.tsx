...
export const CustomerHistoryModal: React.FC<CustomerHistoryModalProps> = ({ isOpen, onClose, customerName, reservations }) => {
  if (!isOpen) return null;

  // Sort by date descending (Newest first) to see recent history easily
  // Or Ascending? Usually history implies past -> future or future -> past.
  // Let's go with Date Descending (Future -> Past) so upcoming is at top, history below.
  ...
  const sortedDates = Object.keys(grouped).sort((a, b) => b.localeCompare(a)); // Descending dates
        ...
        <div className="px-6 py-5 bg-gradient-to-r from-slate-800 to-slate-900 border-b border-slate-700 flex justify-between items-center shrink-0">
          <div>
            <h2 className="text-xl md:text-2xl font-bold text-white flex items-center gap-2">
               🔍 <span className="text-blue-400">{customerName}</span>님 예약 내역
               <span className="text-sm font-normal text-slate-400 bg-slate-800 px-2 py-0.5 rounded-full border border-slate-700 ml-2">
                 총 {totalCount}건
               </span>
            </h2>
            <p className="text-slate-400 text-xs md:text-sm mt-1">해당 고객의 전체 예약 히스토리입니다.</p>
          </div>
          <button 
            onClick={onClose} 
                ...
                return (
                    <div key={date} className={`relative pl-1 ${idx !== 0 ? 'mt-8' : ''}`}>
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            {grouped[date].sort((a, b) => b.time.localeCompare(a.time)).map(r => ( // Latest time first within day
                                <div 
                                    key={r.id} 
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0 flex flex-col justify-center gap-1.5">
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 text-xs text-slate-400">
                                            {r.customerPhone && (
                                                <a href={`tel:${r.customerPhone}`} className="flex items-center gap-1.5 hover:text-blue-400 transition-colors">
